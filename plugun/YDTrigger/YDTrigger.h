...
#  define YDWEGetObjectProperty(object_type, object_id, property)            \
     YDTRIGGER_COMMON_SWITCH(YDWE_GET_OBJECT_PROPERTY_ ## object_type, (     \
       EXExecuteScript("(require'YDWEXweiObjectSlk').ability("      + I2S(object_id) + ", '" + property + "')"), \
       EXExecuteScript("(require'YDWEXweiObjectSlk').buff("         + I2S(object_id) + ", '" + property + "')"), \
       EXExecuteScript("(require'YDWEXweiObjectSlk').unit("         + I2S(object_id) + ", '" + property + "')"), \
       EXExecuteScript("(require'YDWEXweiObjectSlk').item("         + I2S(object_id) + ", '" + property + "')"), \
       EXExecuteScript("(require'YDWEXweiObjectSlk').upgrade("      + I2S(object_id) + ", '" + property + "')"), \
       EXExecuteScript("(require'YDWEXweiObjectSlk').doodad("       + I2S(object_id) + ", '" + property + "')"), \
       EXExecuteScript("(require'YDWEXweiObjectSlk').destructable(" + I2S(object_id) + ", '" + property + "')"), \
       ))
...
#  include <YDTrigger/Frame.h>
#  include <YDTrigger/commandfunc.h>
#  include <../jass/package/insert.j>
#  include <../jass/popo/Other.j>
